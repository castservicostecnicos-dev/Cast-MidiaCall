...
export interface User {
  ...
  active: boolean;
  operator_type?: 'reception' | 'consultorio';
...
export interface Player {
  ...
  status: 'active' | 'inactive';
  access_token: string; // Permanent unique access token for direct 1-click loading without password/code
  last_seen: string;
  created_at: string;
  updated_at: string;
}
...
export interface PlayerCall {
  ...
  call_source?: 'reception' | 'consultorio';
  duration: number;
  is_priority?: boolean;
  chime_id?: string;
  chime_url?: string;
...
export interface AlertSound {
  ...
  type: 'system' | 'custom';
  audio_url?: string;
  preset_tone?: 'default' | 'bell' | 'digital' | 'marimba' | 'soft' | 'hospital';
  is_default?: boolean;
  company_id?: string | null;
  created_at?: string;
}

export const DEFAULT_ALERT_SOUNDS: AlertSound[] = [
  {
    id: 'chime-default',
    name: 'Chime Padrão CAST MidiaCall',
    description: 'Campainha harmônica em 2 tempos (3 tempos para preferencial).',
    type: 'system',
    preset_tone: 'default',
    is_default: true,
    company_id: null,
  ...
  {
    id: 'chime-bell',
    name: 'Sino de Recepção Suave',
    description: 'Tom clássico de recepção de hotel / balcão de atendimento.',
    type: 'system',
    preset_tone: 'bell',
    is_default: false,
    company_id: null,
  ...
  {
    id: 'chime-digital',
    name: 'Gongo Eletrônico Triplo',
    description: 'Chime dinâmico e nítido para locais com ruído moderado.',
    type: 'system',
    preset_tone: 'digital',
    is_default: false,
    company_id: null,
  ...
  {
    id: 'chime-marimba',
    name: 'Marimba Acústica',
    description: 'Sonoridade orgânica e agradável em tons de madeira.',
    type: 'system',
    preset_tone: 'marimba',
    is_default: false,
    company_id: null,
  ...
  {
    id: 'chime-soft',
    name: 'Harmonia Zen / Consultório',
    description: 'Alerta discreto e calmo para clínicas, spas e consultórios.',
    type: 'system',
    preset_tone: 'soft',
    is_default: false,
    company_id: null,
  ...
  {
    id: 'chime-hospital',
    name: 'Alerta Clínico / Hospitalar',
    description: 'Bip duplo nítido de alta clareza para ambientes ambulatoriais.',
    type: 'system',
    preset_tone: 'hospital',
    is_default: false,
    company_id: null,
    created_at: '2026-09-01T00:00:00.000Z',
  },
...
/**
 * Returns today's date string in Brazilian/local timezone (YYYY-MM-DD).
 * Ensures daily reset at 00:00h regardless of server UTC time.
 */
...
/**
 * Retrieves the current ticket state for a company, resetting to 0 if the date has changed (00:00h daily rollover).
 * Normal and Preferential queues have their own independent sequences (001 to 999).
 */
export function getCompanyTicketState(
  companyId: string,
  data: DatabaseSchema
): {
  normal_number: number;
  ...
  if (!company.ticket_counter || company.ticket_counter.last_reset_date !== today) {
    company.ticket_counter = {
      normal_number: 0,
      priority_number: 0,
      current_number: 0,
      last_reset_date: today,
    ...
    // Migration: ensure both normal_number and priority_number exist
    if (typeof company.ticket_counter.normal_number !== 'number') {
      company.ticket_counter.normal_number = company.ticket_counter.current_number || 0;
    }
    if (typeof company.ticket_counter.priority_number !== 'number') {
      company.ticket_counter.priority_number = 0;
    }
    if (typeof company.ticket_counter.current_number !== 'number') {
      company.ticket_counter.current_number = company.ticket_counter.normal_number;
    }
  }

  return company.ticket_counter as {
    normal_number: number;
    priority_number: number;
    current_number: number;
    last_reset_date: string;
...
/**
 * Allocates the next sequential ticket (001 to 999, resets to 001 after 999).
 * - Normal tickets follow their own sequence (001 to 999).
 * - Preferential tickets follow their own sequence (P001 to P999).
 * - Calling a normal ticket does NOT change the preferential counter.
 * - Calling a preferential ticket does NOT change the normal counter.
 */
export function allocateNextCompanyTicket(
  companyId: string,
  data: DatabaseSchema,
  ...
  if (isPriority) {
    // Next number in priority sequence 1..999, wrapping after 999 back to 1
    const nextNum = ((state.priority_number || 0) % 999) + 1;
    state.priority_number = nextNum;
    state.current_number = nextNum;
    state.updated_at = new Date().toISOString();

    const formattedPure = String(nextNum).padStart(3, '0');
    const formatted = `P${formattedPure}`;

    return {
      number: nextNum,
      formatted,
      formattedPure,
      normalNumber: state.normal_number,
      priorityNumber: state.priority_number,
    };
  } else {
    // Next number in normal sequence 1..999, wrapping after 999 back to 1
    const nextNum = ((state.normal_number || 0) % 999) + 1;
    state.normal_number = nextNum;
    state.current_number = nextNum;
    state.updated_at = new Date().toISOString();

    const formattedPure = String(nextNum).padStart(3, '0');
    const formatted = formattedPure;
...
/**
 * Manually resets the company ticket counter back to 0 (next will be 001).
 * Can reset all, only normal, or only priority.
 */
export function resetCompanyTicketCounter(
  companyId: string,
  data: DatabaseSchema,
  target: 'all' | 'normal' | 'priority' = 'all'
): {
  normal_number: number;
  priority_number: number;
  current_number: number;
  ...
  if (target === 'normal') {
    state.normal_number = 0;
  } else if (target === 'priority') {
    state.priority_number = 0;
  } else {
    state.normal_number = 0;
    state.priority_number = 0;
...
export const DEFAULT_PLANS: Plan[] = [
  ...
  },
  {
    id: 'plan-show-basic',
    name: 'Show Básico',
    description: 'Exibição CAST MidiaCall, propagandas, hora certa e notícias RSS em até 2 telas (sem operador).',
    max_players: 2,
    max_operators: 0,
    max_storage: 50,
    monthly_price: 29.0,
...
class DatabaseStore {
  ...
  constructor() {
    ...
    if (fs.existsSync(dbPath)) {
      try {
        ...
        // Normalize player orientation and ensure permanent unique access_token
        let changed = false;
        if (this.data.players) {
          for (const p of this.data.players) {
            ...
            if (!p.access_token) {
              p.access_token = crypto.randomBytes(24).toString('hex');
              changed = true;
            }
          }
        }
        ...
        if (!this.data.drive_settings) {
          this.data.drive_settings = {
            connected: false,
            root_folder_name: 'CAST MIDIACALL - ARQUIVOS DO SISTEMA',
          };
          changed = true;
        }
        if (!this.data.alert_sounds || this.data.alert_sounds.length === 0) {
          this.data.alert_sounds = [...DEFAULT_ALERT_SOUNDS];
          changed = true;
        } else {
          // Ensure all default system chimes exist
          for (const defSound of DEFAULT_ALERT_SOUNDS) {
            if (!this.data.alert_sounds.some((s) => s.id === defSound.id)) {
              this.data.alert_sounds.push(defSound);
              changed = true;
            }
          }
        }

        // Ensure ticket_counter exists on all companies
        if (this.data.companies) {
          const today = getLocalTodayDateString();
          for (const comp of this.data.companies) {
            if (!comp.ticket_counter) {
              comp.ticket_counter = {
                normal_number: 0,
                priority_number: 0,
                current_number: 0,
              ...
              if (typeof comp.ticket_counter.normal_number !== 'number') {
                comp.ticket_counter.normal_number = comp.ticket_counter.current_number || 0;
                changed = true;
              }
              if (typeof comp.ticket_counter.priority_number !== 'number') {
                comp.ticket_counter.priority_number = 0;
                changed = true;
              }
            }
          }
        ...
        // Ensure operators have operator_type defined ('reception' or 'consultorio')
        if (this.data.operators) {
          for (const op of this.data.operators) {
            if (!op.operator_type) {
              op.operator_type = op.email.includes('consultorio') ? 'consultorio' : 'reception';
              changed = true;
            }
          }
        }

        // Ensure demo company has a consultório operator available
        const demoComp = this.data.companies?.find((c) => c.id === 'comp-demo-1');
        if (demoComp) {
          const hasConsultorio = this.data.operators?.some(
            (o) => o.company_id === demoComp.id && o.operator_type === 'consultorio'
          );
          if (!hasConsultorio) {
            const demoPass = hashPassword('123456');
            const now = new Date().toISOString();
            const consultorioUser: User = {
              id: 'usr-op-consultorio-1',
              name: 'Dr. Roberto - Consultório 01',
              email: 'consultorio@drogariasp.com.br',
              password_hash: demoPass.hash,
              salt: demoPass.salt,
              role: 'operator',
              company_id: demoComp.id,
              active: true,
              must_change_password: false,
              created_at: now,
              updated_at: now,
            };
            this.data.users.push(consultorioUser);

            this.data.operators.push({
              id: 'op-consultorio-1',
              company_id: demoComp.id,
              user_id: consultorioUser.id,
              name: 'Dr. Roberto - Consultório 01',
              email: 'consultorio@drogariasp.com.br',
              phone: '(11) 98765-4322',
              operator_type: 'consultorio',
              active: true,
              created_at: now,
              updated_at: now,
            });
  ...
  private createInitialData(): DatabaseSchema {
    ...
    const players: Player[] = [
      {
        ...
        playlist_id: playlistId,
        status: 'active',
        access_token: 'tok_play_rec_01_' + crypto.randomBytes(12).toString('hex'),
      ...
      },
      {
        ...
        playlist_id: playlistId,
        status: 'active',
        access_token: 'tok_play_sala_02_' + crypto.randomBytes(12).toString('hex'),
    ...
    return {
      users: [adminUser, adminUser2, companyUser, operatorUser, playerUser1, playerUser2],
      companies: [company],
      plans,
      players,
      operators: [operator],
      ...
      alert_sounds: [...DEFAULT_ALERT_SOUNDS],
    };
  }
  ...
  // ==========================================
  // ALERT SOUNDS (CHIMES DE CHAMADA DE SENHAS)
  // ==========================================
  public getAlertSounds(companyId?: string): AlertSound[] {
    if (!this.data.alert_sounds) {
      this.data.alert_sounds = [...DEFAULT_ALERT_SOUNDS];
      this.save();
    }
    // Return system chimes + company custom chimes
    return this.data.alert_sounds.filter((s) => !s.company_id || (companyId && s.company_id === companyId));
  }

  public getAlertSound(id: string): AlertSound | undefined {
    return (this.data.alert_sounds || []).find((s) => s.id === id);
  }

  public createAlertSound(data: Omit<AlertSound, 'id' | 'created_at'>): AlertSound {
    if (!this.data.alert_sounds) {
      this.data.alert_sounds = [...DEFAULT_ALERT_SOUNDS];
    }
    const id = `chime-custom-${Date.now()}-${Math.random().toString(36).substring(2, 7)}`;
    const newSound: AlertSound = {
      ...
      preset_tone: data.preset_tone,
      is_default: Boolean(data.is_default),
      company_id: data.company_id || null,
      created_at: new Date().toISOString(),
    };
    this.data.alert_sounds.push(newSound);
    this.save();
    return newSound;
  }

  public deleteAlertSound(id: string, companyId?: string): boolean {
    if (!this.data.alert_sounds) return false;
    const initial = this.data.alert_sounds.length;
    // Cannot delete system sounds
    this.data.alert_sounds = this.data.alert_sounds.filter(
      (s) => !(s.id === id && s.type === 'custom' && (!companyId || s.company_id === companyId))
    );
    if (this.data.alert_sounds.length !== initial) {
      this.save();
      return true;
    }
    return false;
