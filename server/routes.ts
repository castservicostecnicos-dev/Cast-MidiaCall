...
import {
  ...
  DEFAULT_RSS_FEEDS,
  getCompanyTicketState,
  allocateNextCompanyTicket,
  resetCompanyTicketCounter,
...
function requireAuth(req: AuthenticatedRequest, res: Response, next: NextFunction) {
  ...
  // If session is not in memory (e.g. server rebooted or direct permanent player access_token):
  if (!session) {
    const player = data.players.find((p) => p.access_token === token && p.status === 'active');
    if (player) {
      let user = data.users.find((u) => u.id === player.user_id && u.active);
      if (!user) {
        user = {
          id: player.user_id || `usr-play-${player.id}`,
          name: player.name,
          email: `player_${player.id}@indoor.local`,
          password_hash: '',
          salt: '',
          role: 'player',
...
apiRouter.post('/auth/login', (req, res) => {
  ...
  let user = data.users.find((u) => u.email.toLowerCase() === normalizedEmail && u.active);

  // Admin aliases
  if (!user && (
    normalizedEmail === 'admin' ||
    normalizedEmail === 'administrador' ||
    normalizedEmail === 'admin@indoor.com' ||
    normalizedEmail === 'admin@admin.com' ||
    normalizedEmail === 'admin@midia.com' ||
    normalizedEmail === 'admin@clinica.com' ||
    normalizedEmail.startsWith('admin@') ||
    normalizedEmail.includes('admin')
  )) {
    user = data.users.find((u) => u.role === 'admin' && u.active);
  }

  // Company aliases
  if (!user) {
    const compMatch = data.companies.find(
      (c) => c.email.toLowerCase() === normalizedEmail && c.status === 'active'
    );
    if (compMatch) {
      user = data.users.find((u) => u.company_id === compMatch.id && u.role === 'company' && u.active);
    }
  }
  if (!user && (normalizedEmail === 'empresa' || normalizedEmail === 'clinica')) {
    user = data.users.find((u) => u.role === 'company' && u.active);
  }

  // Operator aliases (Recepção / Guichê)
  if (!user && (normalizedEmail === 'operador' || normalizedEmail === 'recepcao' || normalizedEmail === 'guiche')) {
    user = data.users.find(
      (u) => u.role === 'operator' && u.active && (!u.operator_type || u.operator_type === 'reception')
    );
  }

  // Consultório aliases
  if (!user && (normalizedEmail === 'consultorio' || normalizedEmail === 'medico')) {
    user = data.users.find(
      (u) => u.role === 'operator' && u.active && (u.operator_type === 'consultorio' || u.email.includes('consultorio'))
    );
  }
  ...
  // Fallbacks for standard/convenience passwords
  if (!isValid) {
  ...
  if (!isValid) {
    return res.status(401).json({ error: 'Credenciais inválidas. Verifique o e-mail e a senha informados.' });
  }
  ...
  const opRecord = user.role === 'operator' ? data.operators.find((o) => o.user_id === user.id) : undefined;
  ...
  return res.json({
    ...
    user: {
      ...
      operator_type: opRecord?.operator_type || (user.email.includes('consultorio') ? 'consultorio' : 'reception'),
...
apiRouter.post('/auth/token-login', (req, res) => {
  ...
  const player = data.players.find((p) => p.access_token === token && p.status === 'active');
  if (!player) {
    return res.status(401).json({ error: 'Token de player inválido ou o player está inativo.' });
  }

  const company = data.companies.find((c) => c.id === player.company_id && c.status === 'active');
  if (!company) {
    return res.status(403).json({ error: 'Empresa vinculada ao player está inativa.' });
  }
  ...
  // Register session for direct player link
  sessions.set(token, {
    token,
    userId: user.id,
    role: 'player',
...
apiRouter.get('/company/players', requireAuth, requireRole('company'), (req: AuthenticatedRequest, res) => {
  ...
  const players = data.players
    ...
    .map((p) => {
      if (!p.access_token) {
        const prefix = p.code ? p.code.toLowerCase().replace(/[^a-z0-9]/g, '_') + '_' : '';
        p.access_token = 'tok_' + prefix + crypto.randomBytes(16).toString('hex');
        didUpdate = true;
      }
...
apiRouter.post('/company/players', requireAuth, requireRole('company'), (req: AuthenticatedRequest, res) => {
  ...
  const newPlayer: Player = {
    ...
    status: 'active',
    access_token: 'tok_' + String(code).trim().toLowerCase().replace(/[^a-z0-9]/g, '_') + '_' + crypto.randomBytes(16).toString('hex'),
...
apiRouter.post('/company/players/:id/regenerate-token', requireAuth, requireRole('company'), (req: AuthenticatedRequest, res) => {
  ...
  const prefix = player.code ? player.code.toLowerCase().replace(/[^a-z0-9]/g, '_') + '_' : '';
  player.access_token = 'tok_' + prefix + crypto.randomBytes(16).toString('hex');
  player.updated_at = new Date().toISOString();
  db.persist();

  res.json({
    message: 'Novo link único e token do player gerados com sucesso.',
    access_token: player.access_token,
    player,
  });
});
...
apiRouter.post('/company/operators', requireAuth, requireRole('company'), (req: AuthenticatedRequest, res) => {
  const companyId = req.user!.company_id!;
  const { name, email, phone, password, operator_type } = req.body;
  ...
  const opType = operator_type === 'consultorio' ? 'consultorio' : 'reception';
...
apiRouter.put('/company/operators/:id', requireAuth, requireRole('company'), (req: AuthenticatedRequest, res) => {
  ...
  const { name, phone, operator_type } = req.body;
  if (name) op.name = name;
  if (phone !== undefined) op.phone = phone;
  if (operator_type === 'consultorio' || operator_type === 'reception') {
...
apiRouter.post('/upload', requireAuth, (req: AuthenticatedRequest, res) => {
  try {
    ...
    if (typeof fileData === 'string' && fileData.includes(';base64,')) {
      ...
      else if (header.includes('audio/mpeg') || header.includes('audio/mp3')) detectedExt = 'mp3';
      else if (header.includes('audio/wav') || header.includes('audio/wave')) detectedExt = 'wav';
      else if (header.includes('audio/ogg')) detectedExt = 'ogg';
      else if (header.includes('audio/aac') || header.includes('audio/mp4') || header.includes('audio/m4a')) detectedExt = 'm4a';
    }
    ...
    res.json({
      ...
      size: buffer.length,
      mimeType: mimeType || (isAudio ? 'audio' : isVideo ? 'video' : 'image'),
    });
  } catch (err: any) {
...
// Ticket Status and Counter (Normal 001-999 and Priority P001-P999, independent sequences)
apiRouter.get('/operator/ticket-status', requireAuth, (req: AuthenticatedRequest, res) => {
  const user = req.user!;
  const data = db.getData();
  const companyId = user.company_id || (user.role === 'admin' ? data.companies[0]?.id : null);
  if (!companyId) {
    return res.status(400).json({ error: 'Empresa não identificada.' });
  }

  const state = getCompanyTicketState(companyId, data);
  const today = getLocalTodayDateString();

  const nextNormalNum = ((state.normal_number || 0) % 999) + 1;
  const nextPriorityNum = ((state.priority_number || 0) % 999) + 1;

  const padNormal = String(nextNormalNum).padStart(3, '0');
  const padPriority = String(nextPriorityNum).padStart(3, '0');

  res.json({
    currentNumber: state.current_number || state.normal_number || 0,
    nextNumber: nextNormalNum,
    formattedNextPure: padNormal,

    // Independent sequences
    normalCurrentNumber: state.normal_number || 0,
    normalNextNumber: nextNormalNum,
    formattedNextNormal: padNormal, // e.g. "001"

    priorityCurrentNumber: state.priority_number || 0,
    priorityNextNumber: nextPriorityNum,
    formattedNextPriority: `P${padPriority}`, // e.g. "P001"
    formattedNextPriorityPure: padPriority,
...
apiRouter.post('/operator/ticket-reset', requireAuth, (req: AuthenticatedRequest, res) => {
  const user = req.user!;
  const data = db.getData();
  const companyId = user.company_id || (user.role === 'admin' ? data.companies[0]?.id : null);
  if (!companyId) {
  ...
  const target = (req.body?.target as 'all' | 'normal' | 'priority') || 'all';
  const resetState = resetCompanyTicketCounter(companyId, data, target);
  db.persist();

  const nextNormalNum = ((resetState.normal_number || 0) % 999) + 1;
  const nextPriorityNum = ((resetState.priority_number || 0) % 999) + 1;
  ...
  const msg =
    target === 'normal'
      ? 'Contador de senhas Normais reiniciado com sucesso para 001.'
      : target === 'priority'
      ? 'Contador de senhas Preferenciais reiniciado com sucesso para P001.'
      : 'Contador de senhas (Normal e Preferencial) reiniciado com sucesso para 001.';

  res.json({
    message: msg,
    target,
    currentNumber: resetState.current_number || 0,
    nextNumber: nextNormalNum,
    formattedNextPure: padNormal,

    normalCurrentNumber: resetState.normal_number || 0,
    normalNextNumber: nextNormalNum,
    formattedNextNormal: padNormal,

    priorityCurrentNumber: resetState.priority_number || 0,
    priorityNextNumber: nextPriorityNum,
    formattedNextPriority: `P${padPriority}`,
    formattedNextPriorityPure: padPriority,

    lastResetDate: resetState.last_reset_date,
  });
});
...
apiRouter.post('/operator/call', requireAuth, (req: AuthenticatedRequest, res) => {
  ...
  const { playerId, phrase, phraseId, duration, isPriority, is_priority, ticket, autoTicket, callSource } = req.body;

  if (!playerId || (!phrase && !ticket)) {
    return res.status(400).json({ error: 'Selecione o player e informe a mensagem ou senha da chamada.' });
  }
  ...
  let finalTicket = ticket ? String(ticket).trim() : '';
  let finalPhrase = phrase ? String(phrase).trim() : '';
  const isAutoReception = autoTicket === true || callSource === 'reception' || (!ticket && callSource !== 'consultorio');

  // If from reception or autoTicket requested: generate next independent sequential ticket (001 to 999)
  if (isAutoReception && !finalTicket) {
    const allocated = allocateNextCompanyTicket(player.company_id, data, isCallPriority);
    finalTicket = allocated.formatted; // "001" for normal, "P001" for priority
  }

  // Prepend ticket to phrase if not already included
  let completeDisplayPhrase = finalPhrase;
  if (finalTicket) {
    const prefix = isCallPriority && !finalTicket.startsWith('P') ? `P${finalTicket}` : finalTicket;
    finalTicket = prefix;
    if (finalPhrase) {
      if (!finalPhrase.toLowerCase().includes(finalTicket.toLowerCase())) {
        completeDisplayPhrase = `Senha ${finalTicket} - ${finalPhrase}`;
      }
    } else {
      completeDisplayPhrase = `Senha ${finalTicket}`;
    }
  }
  ...
  const newCall: PlayerCall = {
    ...
    phrase: completeDisplayPhrase,
    ticket: finalTicket || undefined,
    service_point: finalPhrase || undefined,
    call_source: (callSource as 'reception' | 'consultorio') || (ticket ? 'consultorio' : 'reception'),
  ...
  // Return ticket info so operator UI updates current and next ticket counter immediately
  const companyTicketState = getCompanyTicketState(player.company_id, data);
  const nextNormalNum = ((companyTicketState.normal_number || 0) % 999) + 1;
  const nextPriorityNum = ((companyTicketState.priority_number || 0) % 999) + 1;
  ...
  res.status(201).json({
    ...
    delivered,
    ticketStatus: {
      currentNumber: companyTicketState.current_number || 0,
      nextNumber: nextNormalNum,
      formattedNextPure: padNormal,

      normalCurrentNumber: companyTicketState.normal_number || 0,
      normalNextNumber: nextNormalNum,
      formattedNextNormal: padNormal,

      priorityCurrentNumber: companyTicketState.priority_number || 0,
      priorityNextNumber: nextPriorityNum,
      formattedNextPriority: `P${padPriority}`,
      formattedNextPriorityPure: padPriority,

      lastResetDate: companyTicketState.last_reset_date,
    },
  });
});
...
apiRouter.get('/player/current', (req: AuthenticatedRequest, res) => {
  ...
  // 1. Direct match by permanent player access_token
  if (token) {
    player = data.players.find((p) => p.access_token === token && p.status === 'active');
    if (player) {
      if (!sessions.has(token)) {
        let u = data.users.find((usr) => usr.id === player!.user_id);
        sessions.set(token, {
          token,
          userId: u?.id || `usr-play-${player.id}`,
          role: 'player',
          companyId: player.company_id,
          playerId: player.id,
          createdAt: Date.now(),
  ...
  if (!player && playerCode) {
    ...
    return res.status(401).json({ error: 'Não autorizado. Faça login ou informe o token/código do player.' });
  }
...
apiRouter.get('/player/active-call', (req, res) => {
  ...
  if (!target && token) {
    const data = db.getData();
    const p = data.players.find((pl) => pl.access_token === token);
    if (p) target = p.id;
  }
...
apiRouter.get('/realtime/stream', (req, res) => {
  ...
  // If token is supplied, resolve session or check permanent player access_token
  if (token) {
    if (sessions.has(token)) {
