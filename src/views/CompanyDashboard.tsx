...
export const CompanyDashboard: React.FC<CompanyDashboardProps> = ({
  ...
  const [playerLinkModal, setPlayerLinkModal] = useState<{
    isOpen: boolean;
    player: Player | null;
  ...
  const getPlayerDirectLink = (player: Player) => {
  ...
  const handleCopyPlayerLink = async (player: Player) => {
    const link = getPlayerDirectLink(player);
    try {
      ...
      showToast('success', `Link único do player "${player.name}" copiado com sucesso!`);
      setTimeout(() => setCopiedPlayerId(null), 2500);
    } catch {
      showToast('error', 'Falha ao copiar link para a área de transferência.');
    }
  };

  const handleRegeneratePlayerToken = async (player: Player) => {
    try {
      setRegeneratingToken(true);
      const res = await api.regeneratePlayerToken(player.id);
      setPlayers((prev) =>
        prev.map((p) => (p.id === player.id ? { ...p, access_token: res.access_token } : p))
      );
      if (playerLinkModal.player?.id === player.id) {
        setPlayerLinkModal((prev) => ({
          ...prev,
          player: { ...prev.player!, access_token: res.access_token },
        }));
      }
      showToast('success', 'Novo link de acesso e token gerados com sucesso!');
    } catch (err: any) {
      showToast('error', err.message || 'Erro ao gerar novo token.');
    } finally {
  ...
  const handleOpenOperatorModal = (operator?: Operator) => {
    if (operator) {
      ...
      setOperatorForm({
        ...
        password: '',
        operator_type: operator.operator_type || 'reception',
      });
    } else {
  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-8 sm:px-6">
            ...
            {players.length === 0 ? (
              ...
              players.map((p) => (
                <div key={p.id} className="rounded-xl border border-slate-700 bg-slate-800 p-4 shadow-sm space-y-3">
                  ...
                  {/* Link de Atalho Único (1-clique sem login) */}
                  <div className="rounded-lg bg-blue-950/30 border border-blue-800/50 p-2.5 space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1.5 text-blue-300 font-bold text-[11px] uppercase tracking-wider">
                        <Link2 className="h-3.5 w-3.5" />
                        <span>Link Único (Atalho TV)</span>
                      </div>
                      <button
                        onClick={() => setPlayerLinkModal({ isOpen: true, player: p })}
                    ...
                    <div className="flex items-center gap-1.5">
                      <input
                        type="text"
                        readOnly
                        value={getPlayerDirectLink(p)}
                        className="flex-1 min-h-[38px] rounded border border-slate-700 bg-slate-900 px-2 text-[11px] font-mono text-emerald-400 truncate select-all"
                      />
                      <button
                        onClick={() => handleCopyPlayerLink(p)}
                        className={`min-h-[38px] px-3 rounded text-xs font-bold transition cursor-pointer flex items-center gap-1 shrink-0 ${
                          copiedPlayerId === p.id
                            ? 'bg-emerald-600 text-white'
                            : 'bg-blue-600 hover:bg-blue-500 text-white'
                        }`}
                      >
                        {copiedPlayerId === p.id ? <Check className="h-3.5 w-3.5" /> : <Copy className="h-3.5 w-3.5" />}
                        <span>{copiedPlayerId === p.id ? 'Copiado!' : 'Copiar'}</span>
                      </button>
                    </div>
                  </div>
            ...
            <table className="w-full text-left text-xs text-slate-300">
              <thead className="border-b border-slate-700 bg-slate-800 uppercase font-bold text-slate-400 text-[10px] tracking-wider">
                <tr>
                  ...
                  <th className="px-5 py-3.5">Link Único (Atalho TV)</th>
                    ...
                    <tr key={p.id} className="hover:bg-slate-700/30 transition">
                      ...
                      <td className="px-5 py-4">
                        <div className="flex items-center gap-1.5">
                          <button
                            onClick={() => handleCopyPlayerLink(p)}
                            className={`px-2.5 py-1.5 rounded-md text-xs font-bold transition cursor-pointer flex items-center gap-1.5 shadow-sm shrink-0 ${
                              copiedPlayerId === p.id
                                ? 'bg-emerald-600 text-white'
                                : 'bg-blue-600/20 hover:bg-blue-600/30 text-blue-300 border border-blue-700/60'
                            }`}
                            title="Copiar link direto para abrir no aparelho sem login nem senha"
                          >
                            {copiedPlayerId === p.id ? (
                              <>
                          ...
                          <button
                            onClick={() => setPlayerLinkModal({ isOpen: true, player: p })}
                            className="p-1.5 rounded-lg text-slate-400 hover:text-blue-300 hover:bg-slate-700 transition cursor-pointer"
                            title="Ver opções do atalho e gerar novo token"
                          >
                      ...
                      <div className="flex items-center gap-2 flex-wrap">
                        ...
                        <span
                          className={`text-[10px] px-2 py-0.5 rounded border font-semibold ${
                            op.operator_type === 'consultorio'
                              ? 'bg-amber-950/80 text-amber-300 border-amber-800'
                              : 'bg-blue-950/80 text-blue-300 border-blue-800'
                          }`}
                        >
                          {op.operator_type === 'consultorio' ? '🩺 Consultório' : '🛎️ Guichê / Recepção'}
                        </span>
                      </div>
            ...
            <table className="w-full text-left text-xs text-slate-300">
              <thead className="border-b border-slate-700 bg-slate-800 uppercase font-bold text-slate-400 text-[10px] tracking-wider">
                <tr>
                  ...
                  <th className="px-5 py-3.5">Tipo de Posto</th>
          ...
          <div className="w-full max-w-md rounded-t-2xl sm:rounded-2xl border border-slate-800 bg-slate-900 shadow-2xl text-slate-100 flex flex-col max-h-[92vh] sm:max-h-[85vh] animate-in slide-in-from-bottom sm:slide-in-from-bottom-0">
            ...
            <form onSubmit={handleSavePlayer} className="flex flex-col flex-1 min-h-0">
              <div className="flex-1 overflow-y-auto p-5 space-y-4 text-xs">
                ...
                {editingPlayer && (
                  <div className="rounded-xl bg-blue-950/40 border border-blue-800/60 p-3.5 space-y-2">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-1.5 text-blue-300 font-bold text-[11px] uppercase tracking-wider">
                        <Link2 className="h-3.5 w-3.5" />
                        <span>Link Único de Atalho</span>
                      </div>
                      <button
                        type="button"
                        onClick={() => handleCopyPlayerLink(editingPlayer)}
                        className="text-[10px] font-bold text-white bg-blue-600 hover:bg-blue-500 px-2.5 py-1 rounded flex items-center gap-1 cursor-pointer"
                      >
                        {copiedPlayerId === editingPlayer.id ? <Check className="h-3 w-3" /> : <Copy className="h-3 w-3" />}
                        <span>{copiedPlayerId === editingPlayer.id ? 'Copiado!' : 'Copiar'}</span>
                      </button>
                    </div>
                    <input
                      type="text"
                      readOnly
                      value={getPlayerDirectLink(editingPlayer)}
                      className="w-full rounded-lg border border-slate-700 bg-slate-900 px-2.5 py-2 font-mono text-[10px] text-emerald-400 select-all"
                    />
                    <p className="text-[10px] text-slate-400">
                      Abra este link em qualquer Smart TV ou crie um atalho para abrir o player automaticamente sem senha.
                    </p>
                  </div>
                )}
              </div>
      ...
      {/* MODAL LINK DO PLAYER (ATALHO ÚNICO DE ACESSO DIRETO) */}
      {playerLinkModal.isOpen && playerLinkModal.player && (
        <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/80 p-0 sm:p-4 backdrop-blur-xs">
          <div className="w-full max-w-lg rounded-t-2xl sm:rounded-2xl border border-slate-800 bg-slate-900 shadow-2xl text-slate-100 flex flex-col max-h-[92vh] sm:max-h-[85vh] animate-in slide-in-from-bottom sm:slide-in-from-bottom-0">
            ...
            <div className="shrink-0 px-5 py-3 sm:py-4 border-b border-slate-800 flex items-center justify-between">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-base font-bold text-white">
                    Link Único do Player (Atalho TV)
                  </h3>
                  <p className="text-[11px] text-slate-400">
                    {playerLinkModal.player.name} • <span className="font-mono text-blue-400">{playerLinkModal.player.code}</span>
                  </p>
                </div>
              </div>
              <button
                type="button"
                onClick={() => setPlayerLinkModal({ isOpen: false, player: null })}
                className="text-slate-400 hover:text-white p-1.5 rounded-lg hover:bg-slate-800 transition cursor-pointer"
              >
                <X className="h-5 w-5" />
              </button>
            ...
            <div className="flex-1 overflow-y-auto p-5 space-y-4 text-xs">
              {/* Informational Card */}
              <div className="rounded-xl border border-blue-800/60 bg-blue-950/40 p-3.5 space-y-2 text-blue-200">
                <div className="flex items-center gap-2 font-bold text-blue-300">
                  <Sparkles className="h-4 w-4 shrink-0 text-amber-400" />
                  <span>Acesso Imediato Sem Senha e Sem Login</span>
                </div>
                <p className="text-[11px] text-slate-300 leading-relaxed">
                  Coloque este link único no navegador da Smart TV, TV Box, Raspberry Pi, Mini PC ou crie um atalho na área de trabalho/tela inicial. Ao clicar ou dar Enter, o player carrega automaticamente já rodando as mídias, vídeos, notícias RSS e sincronizado para receber chamadas de senhas!
                </p>
              </div>
              ...
              <div>
                <label className="block font-semibold text-slate-300 mb-1.5">
                  Link de Acesso Único com Token Integrado:
                </label>
                <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-2">
                  <input
                    type="text"
                    readOnly
                    value={getPlayerDirectLink(playerLinkModal.player)}
                    className="flex-1 min-h-[44px] rounded-lg border border-slate-700 bg-slate-950 px-3 py-2 font-mono text-[11px] text-emerald-400 focus:outline-none select-all"
                  />
                  <button
                    type="button"
                    onClick={() => handleCopyPlayerLink(playerLinkModal.player!)}
                    className={`shrink-0 min-h-[44px] px-4 rounded-lg font-bold text-xs flex items-center justify-center gap-1.5 transition cursor-pointer shadow-sm ${
                      copiedPlayerId === playerLinkModal.player.id
                        ? 'bg-emerald-600 text-white'
                        : 'bg-blue-600 hover:bg-blue-500 text-white'
                    }`}
                  >
              ...
              {/* Action: Open in New Tab to Test */}
              <div className="pt-1">
                <a
                  href={getPlayerDirectLink(playerLinkModal.player)}
                  target="_blank"
                  rel="noreferrer"
                  className="min-h-[42px] w-full flex items-center justify-center gap-2 px-3 py-2 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 font-semibold text-xs transition"
                >
                  <ExternalLink className="h-3.5 w-3.5 text-blue-400" />
                  <span>Testar Atalho em Nova Aba</span>
                </a>
              </div>
              ...
              <div className="pt-3 border-t border-slate-800 space-y-2">
                <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2">
                  <div>
                    <span className="font-semibold text-slate-300 block">Regenerar Token de Acesso</span>
                    <span className="text-[11px] text-slate-400">
                      Caso queira invalidar o link antigo e criar um novo token exclusivo
                    </span>
                  </div>
                  <button
                    type="button"
                    disabled={regeneratingToken}
                    onClick={() => handleRegeneratePlayerToken(playerLinkModal.player!)}
                    className="shrink-0 min-h-[38px] px-3 py-1.5 rounded-lg border border-amber-800/60 bg-amber-950/40 hover:bg-amber-900/40 text-amber-300 text-xs font-semibold flex items-center justify-center gap-1.5 transition cursor-pointer disabled:opacity-50"
                  >
                    <RefreshCw className={`h-3.5 w-3.5 ${regeneratingToken ? 'animate-spin' : ''}`} />
                    <span>{regeneratingToken ? 'Gerando...' : 'Gerar Novo Token'}</span>
                  </button>
                </div>
              </div>
            </div>
            ...
            <div className="shrink-0 p-4 border-t border-slate-800 bg-slate-900/95 flex items-center justify-end">
              <button
                type="button"
                onClick={() => setPlayerLinkModal({ isOpen: false, player: null })}
                className="min-h-[42px] px-5 py-2 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 border border-slate-700 text-xs font-semibold cursor-pointer"
              >
                Fechar
              </button>
            </div>
          ...
          <div className="w-full max-w-md rounded-t-2xl sm:rounded-2xl border border-slate-800 bg-slate-900 shadow-2xl text-slate-100 flex flex-col max-h-[92vh] sm:max-h-[85vh] animate-in slide-in-from-bottom sm:slide-in-from-bottom-0">
            ...
            <form onSubmit={handleSaveOperator} className="flex flex-col flex-1 min-h-0">
              <div className="flex-1 overflow-y-auto p-5 space-y-4 text-xs">
                ...
                <div>
                  <label className="block font-semibold text-slate-300 mb-1.5">Tipo de Posto de Atendimento *</label>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                    <button
                      type="button"
                      onClick={() => setOperatorForm({ ...operatorForm, operator_type: 'reception' })}
                      className={`p-3 rounded-xl border text-left cursor-pointer transition ${
                        operatorForm.operator_type === 'reception'
                          ? 'border-blue-500 bg-blue-950/70 text-white ring-1 ring-blue-500 shadow-xs'
                          : 'border-slate-700 bg-slate-800 text-slate-400 hover:text-white'
                      }`}
                    >
                      <div className="font-bold text-xs flex items-center gap-1.5">
                        <span>🛎️</span>
                        <span>Guichê / Recepção</span>
                      </div>
                      <div className="text-[10px] text-slate-400 mt-1">
                        Gera senhas sequenciais 001 a 999 automáticas.
                      </div>
                    </button>

                    <button
                      type="button"
                      onClick={() => setOperatorForm({ ...operatorForm, operator_type: 'consultorio' })}
                      className={`p-3 rounded-xl border text-left cursor-pointer transition ${
                        operatorForm.operator_type === 'consultorio'
                          ? 'border-blue-500 bg-blue-950/70 text-white ring-1 ring-blue-500 shadow-xs'
                          : 'border-slate-700 bg-slate-800 text-slate-400 hover:text-white'
                      }`}
                    >
                      ...
                      <div className="text-[10px] text-slate-400 mt-1">
                        Digitação de senha do paciente em campo separado.
                      </div>
                    </button>
                  </div>
                </div>
