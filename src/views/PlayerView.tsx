...
import { api, getStoredToken, setStoredToken, getApiUrl } from '../lib/api';
...
export const PlayerView: React.FC<PlayerViewProps> = ({ onExit, overridePlayerCode, overridePlayerToken }) => {
  ...
  const effectiveToken = overridePlayerToken || queryParams?.get('token') || undefined;
  const effectiveCode = overridePlayerCode || queryParams?.get('player') || queryParams?.get('code') || undefined;
  ...
  const DEFAULT_HEADLINES = [
    ...
    'CAST MidiaCall Conectado: Programação digital e chamadas de atendimento em tempo real.',
  ];
  ...
  const loadPlayerData = async () => {
    const offlineCacheKey = `indoor_player_offline_data_${effectiveToken || effectiveCode || 'default'}`;
    try {
      setLoading(true);
      setError(null);
      ...
      try {
        res = await api.getCurrentPlayer(effectiveCode, effectiveToken);
        if (res) {
          try {
            localStorage.setItem(offlineCacheKey, JSON.stringify(res));
            const tokenToSave = effectiveToken || (res.player as any)?.access_token;
            if (tokenToSave) {
              setStoredToken(tokenToSave);
            }
          } catch (e) {}
  ...
  const triggerCallOnScreen = (callItem: any) => {
    ...
    setActiveCall({
      id: callId,
      phrase,
      ticket: callItem.ticket,
      service_point: callItem.service_point,
      call_source: callItem.call_source,
      duration,
  ...
  useEffect(() => {
    const token = effectiveToken || getStoredToken();
    const playerId = data?.player?.id;
    const playerCode = effectiveCode || data?.player?.code;
    ...
    const sseUrl = `${getApiUrl('/api/realtime/stream')}?${params.toString()}`;
  ...
  }, [data?.player?.id, data?.player?.code, data?.company?.id, effectiveCode, effectiveToken]);
  ...
  useEffect(() => {
    ...
    const targetCode = effectiveCode || data?.player?.code;
    const targetToken = effectiveToken || getStoredToken();
  ...
  }, [data?.player?.id, data?.player?.code, effectiveCode, effectiveToken]);
      ...
      <div className="flex h-screen w-screen items-center justify-center bg-slate-950 text-slate-400">
        <div className="flex flex-col items-center gap-3">
          ...
          <p className="text-sm font-semibold tracking-wider uppercase text-slate-300">
            Iniciando Reprodutor CAST MidiaCall...
          </p>
        </div>
      </div>
    );
        ...
        {activeCall && (
          ...
          >
            ...
            <div className="my-auto text-center px-4 w-full max-w-4xl">
              ...
              {(() => {
                let ticketVal = activeCall.ticket;
                let textVal = activeCall.service_point || activeCall.phrase;

                if (!ticketVal && activeCall.phrase) {
                  const m = activeCall.phrase.match(/^senha\s+([A-Z0-9]+)\s*(?:-\s*(.*))?$/i);
                  if (m) {
                    ticketVal = m[1];
                    textVal = m[2] || '';
                  }
                ...
                return (
                  <div className="flex flex-col items-center">
                    {ticketVal && (
                      <div className="mb-4 sm:mb-6 inline-flex flex-col items-center bg-slate-900/90 border-2 border-amber-400/80 rounded-3xl px-8 sm:px-14 py-3 sm:py-5 shadow-2xl">
                        <span className="text-xs sm:text-sm uppercase tracking-widest font-bold text-slate-400">
                          SENHA DE ATENDIMENTO
                        </span>
                        <span className="text-5xl sm:text-7xl lg:text-8xl font-mono font-black text-amber-300 tracking-wider">
                          {ticketVal}
                        </span>
                      </div>
                    )}
