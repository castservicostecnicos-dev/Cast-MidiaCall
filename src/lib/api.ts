import { Role, User, Company, Plan, Player, Operator, Playlist, Media, RssFeed, RssPreset, RssArticle, CallPhrase, PlayerCall, TicketStatus, AdminStats, CompanyStats, WeatherData } from '../types';

const TOKEN_KEY = 'indoor_media_token';

// Base API URL for decoupled deployments (e.g. Cloudflare Pages frontend pointing to external backend API)
const BASE_API_URL = (typeof import.meta !== 'undefined' && import.meta.env?.VITE_API_URL
...
async function request<T>(endpoint: string, options: RequestInit = {}): Promise<T> {
  ...
  const requestUrl = getApiUrl(`/api${endpoint}`);
...
export const api = {
  ...
  tokenLogin: (token: string) =>
    request<{ token: string; user: User; company?: { id: string; name: string }; player?: Player }>('/auth/token-login', {
      method: 'POST',
  ...
  getTicketStatus: () => request<TicketStatus>('/operator/ticket-status'),
  resetTicketCounter: (target?: 'all' | 'normal' | 'priority') =>
    request<TicketStatus & { message: string }>('/operator/ticket-reset', {
      method: 'POST',
  ...
  triggerCall: (data: {
    ...
    ticket?: string;
    autoTicket?: boolean;
    callSource?: 'reception' | 'consultorio';
  }) =>
    request<{ message: string; call: PlayerCall; delivered: boolean; ticketStatus?: TicketStatus }>('/operator/call', {
      method: 'POST',
  ...
  getCurrentPlayer: (code?: string, token?: string) => {
    ...
    return request<{
      player: Pick<Player, 'id' | 'name' | 'code' | 'location' | 'orientation' | 'access_token'>;
