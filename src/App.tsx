...
export default function App() {
  ...
  // DIRECT PLAYER ACCESS VIA URL SHORTCUT (?token=... or ?player=... or ?code=...)
  // Enables 1-click automatic launch on Smart TVs, TV boxes, tablets, kiosks without requiring login or typing code!
  const searchParams = typeof window !== 'undefined' ? new URLSearchParams(window.location.search) : null;
  const urlTokenParam = searchParams?.get('token');
  const urlPlayerParam = searchParams?.get('player') || searchParams?.get('code');

  if (urlTokenParam || urlPlayerParam) {
    return (
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-slate-100 flex flex-col font-sans selection:bg-blue-600 selection:text-white">
      ...
      <main className="flex-1">
        ...
        {user.role === 'operator' && (
          <OperatorDashboard
            showToast={showToast}
            onOpenPlayerSimulation={(code) => setSimulatedPlayerCode(code)}
            operatorType={user.operator_type || 'reception'}
          />
        )}
      </main>
      ...
      <footer className="h-9 bg-slate-800 border-t border-slate-700 px-4 sm:px-8 flex items-center justify-between text-[10px] text-slate-400 uppercase tracking-widest">
        <div>© {new Date().getFullYear()} CAST MidiaCall - v2.4.0</div>
