  ...
  const handleDownloadPdf = () => {
    ...
    try {
      ...
      const drawHeader = (slideNum: number, title: string, subtitle: string) => {
        ...
        doc.setFont('helvetica', 'bold');
        doc.setFontSize(9);
        doc.text('CAST MidiaCall & CALL', pageWidth - 60, 18);
        doc.setFontSize(8);
        doc.setTextColor(textMuted[0], textMuted[1], textMuted[2]);
      ...
      // Title
      doc.setFontSize(26);
      doc.text('CAST MidiaCall Inteligente', 35, 66);
      doc.setFontSize(20);
      ...
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(8.5);
      doc.text('CAST MidiaCall Pura / TV Corporativa', 36, 66);
      ...
      doc.setFont('helvetica', 'normal');
      doc.setFontSize(8.5);
      doc.text('CAST MidiaCall + Chamador de Senhas com Voz', 171, 66);
      ...
      const fileName = clientName
        ? `Apresentacao_CAST_MidiaCall_${clientName.replace(/\s+/g, '_')}.pdf`
        : 'Apresentacao_CAST_MidiaCall_e_Chamadas.pdf';
          ...
          <div className="w-full max-w-4xl aspect-[16/10] sm:aspect-[16/9] rounded-2xl border border-slate-700 bg-slate-900 p-6 sm:p-10 shadow-2xl flex flex-col justify-between transition-all duration-300">
              ...
              <div className="h-full flex flex-col justify-between">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-6 my-auto">
                  ...
                  <div className="rounded-xl border border-emerald-900/60 bg-emerald-950/20 p-5 relative">
                    ...
                    <p className="text-xs text-slate-300 mb-4">
                      Solução completa: CAST MidiaCall aliada à gestão de atendimento com chamador por voz.
                    </p>
                    <ul className="space-y-2 text-xs text-slate-300">
                      <li className="flex items-center gap-2">
            ...
            {currentSlide === 6 && (
              <div className="h-full flex flex-col justify-between">
                ...
                <div className="border-t border-slate-800 pt-2 text-xs text-slate-400 flex justify-between">
                  <span>CAST MidiaCall Inteligente & Painel de Chamadas</span>
